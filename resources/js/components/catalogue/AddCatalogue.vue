    ...
    <v-dialog :value="value" :persistent="loading" @click:outside="onCancel()" @keydown.esc="onCancel()" content-class="modal-dialog">
        <v-card>
            <v-card-title
                ...
                primary-title
            >
                {{isEditMode ? this.$getConst('TXT_UPDATE') : this.$getConst('TXT_ADD')}} Catalogue
            </v-card-title>

            <v-card-text>
                <form method="POST" name="catalogue-form" role="form" novalidate autocomplete="off" @submit.prevent="addAction">
                    <ErrorBlockServer :errorMessage="errorMessage"></ErrorBlockServer>
                    <v-layout row wrap class="display-block m-0 ">
                        <v-flex xs12>
                            <v-text-field
                                label="Catalogue name *" type="text"
                                name="catalogue"
                                v-model="model.name"
                                :error-messages="getErrorValue('catalogue')"
                                v-validate="'required'"
                            ></v-text-field>
                        </v-flex>
...
<script src="./add-catalogue.js"></script>
